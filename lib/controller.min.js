...
this[a].__dispatch(f,b,e);}else{if(this[c] instanceof Function&&this.__parse(f,b,a,c)){var g=domain.create();g.on("error",function(i){try{var h=parseInt(i);
if(!isNaN(h)){b.send(h);}}catch(d){}});g.add(f);g.add(b);this.removeAllListeners("error");this.once("error",g.emit.bind(g,"error"));g.run(this[c].bind(this,f,b));
