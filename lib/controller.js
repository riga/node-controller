...
var Controller = Class._extend({
  ...
  __dispatch: function(req, res, next) {
    ...
    if (this[route] instanceof Controller) {
      ...
      // parse and call the handler
      this.__parse(req, res, route, handler);
      try {
  ...
  __parse: function(req, res, route, handler) {
    ...
    var parNames = this.__parameterNames[route];
  ...
  setParameterNames: function(route, names) {
    var self = this;

    if (route instanceof Object && names == null) {
      Object.keys(route).forEach(function(_route) {
        self.setParameterNames(_route, route[_route]);
      });
      return this;
    }
    ...
    this.__parameterNames[route] = names;
