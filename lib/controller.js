/*!
 * node-controller module v0.2.10
...
var Controller = Emitter._extend({
  ...
  requireParam: function(req, param, err) {
    err = err || 400;

    if (!(param instanceof Array)) {
      if (req.param(param) == null) throw err;
      else return req.param(param);
    } else {
      var values = {};
      param.forEach(function(p) {
        if (req.param(p) == null) throw err;
        else values[p] = req.param(p);
      });
      return values;
    }
  }
